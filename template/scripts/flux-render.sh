...
# the cluster's ConfigMaps and derive the kubeconform schema version.
#
...
#
#   VARS=$(scripts/flux-render.sh export-versions "$CMS") || exit 1
#   eval "$VARS"          # exports every data key + FLUX_ENVSUBST_VARS
#   K8S_VER=$(scripts/flux-render.sh k8s-version "$CMS")
#
...
#
# SUPERSET of the weisssrv-lib copy: a cluster from this template has TWO
# postBuild sources — cluster-versions (pins, generated from the inventory) and
# cluster-config (site identity) — so both subcommands take several ConfigMaps,
# as separate arguments or whitespace-separated inside one argument (the form
# ci/validate/flux-lint.yml's `versions_configmap` input passes). Re-vendoring
# the single-path library copy over this file breaks `task flux:lint` and CI.
# A key present in more than one ConfigMap is an error: Flux merges
# substituteFrom sources in list order, so a duplicate resolves silently to
# whichever source happens to be listed last.
#
...
# Word-split every argument, so one quoted "a.yaml b.yaml" behaves like two.
collect_files() {
    local arg file
    FILES=()
    for arg in "$@"; do
        # shellcheck disable=SC2086  # deliberate word-split of the packed argument
        for file in $arg; do
            [ -f "$file" ] || die "ConfigMap not found: $file"
            FILES+=("$file")
        done
    done
    [ ${#FILES[@]} -gt 0 ] || die "no ConfigMap path given"
}
...
for cm in sys.argv[1:]:
    ...
    if not data:
        sys.exit(f"flux-render: ERROR: no substitution keys found in {cm}")
    for key, value in data.items():
        if not _VALID_KEY.match(key):
            sys.exit(f"flux-render: ERROR: invalid shell variable name in {cm}: {key}")
        if key in source_of:
            sys.exit(f"flux-render: ERROR: key {key} is defined in both {source_of[key]} and {cm}")
        source_of[key] = cm
...
cmd_k8s_version() {
    ...
    ver=$(grep -hE '^ *k3s_version:' "${FILES[@]}" | head -1 | grep -oE '[0-9]+\.[0-9]+' | head -1 || true)
    printf '%s.0\n' "${ver:-1.36}"
}
